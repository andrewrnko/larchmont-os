...
export function BoardSidebar({ collapsed, setCollapsed }: { collapsed: boolean; setCollapsed: (v: boolean) => void }) {
  ...
  const renderBoard = (id: string, depth = 0) => {
    ...
    return (
      <div key={id}>
        <div
          className={`group flex items-center gap-2 rounded px-2 py-1.5 text-[15px] cursor-pointer ${
  ...
  return (
    <div className="flex h-full w-56 flex-col border-r border-[#2a2a2a] bg-[#101010]">
      <div className="flex items-center justify-between border-b border-[#2a2a2a] px-2 py-2">
        <span className="font-mono text-[13px] font-medium uppercase tracking-[0.06em] text-amber-500">Boards</span>
      ...
      {creating && (
        <div className="border-b border-[#2a2a2a] p-2">
          <div className="flex gap-1">
            ...
            <input
              autoFocus
              className="flex-1 rounded bg-[#1a1a1a] px-2 text-[15px] text-white outline-none placeholder:text-neutral-600"
      ...
      <div className="flex-1 overflow-auto p-1">
        {rootBoards.length === 0 && (
          <div className="p-2 text-[15px] text-neutral-600">No boards yet. Click + to create one.</div>
        )}
      ...
      <div className="border-t border-[#2a2a2a] p-2 space-y-1">
        {importStatus && (
          <div className="rounded bg-amber-500/10 px-2 py-1 text-[15px] text-amber-300 text-center">
            {importStatus}
          </div>
        )}
        <div className="flex gap-1">
          <button
            ...
            className="flex flex-1 items-center justify-center gap-1.5 rounded bg-[#1a1a1a] px-2 py-1.5 text-[15px] text-neutral-400 hover:bg-[#222] hover:text-white"
