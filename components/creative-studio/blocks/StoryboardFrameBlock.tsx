  ...
  return (
    <>
      <BlockWrapper block={block} kind="storyboard" onContextMenu={onContextMenu}>
        <div className="flex h-full w-full flex-col overflow-hidden rounded-md border border-[color:rgba(255,255,255,0.07)] bg-[#1c1c1a] shadow-lg">
          {/* Header */}
          <div data-no-drag className="flex items-center justify-between border-b border-[color:rgba(255,255,255,0.07)] bg-[#242422] px-4 py-2">
            <span className="font-mono text-[13px] font-medium uppercase tracking-[0.06em] text-[color:var(--cs-accent)]">Storyboard</span>
            <div className="flex gap-1">
              <button
                className="flex items-center gap-1 rounded bg-[#2d2d2a] px-2 py-1 text-[15px] text-white hover:bg-[color:rgba(255,255,255,0.12)]"
                onClick={(e) => { e.stopPropagation(); sequence() }}
              >
                <Film size={11} /> Sequence
              </button>
              <button
                className="flex items-center gap-1 rounded bg-[color:var(--cs-accent)] px-2 py-1 text-[15px] text-black hover:bg-[color:var(--cs-accent2)]"
          ...
          <div data-no-drag data-scrollable className="flex flex-1 gap-3 overflow-auto p-3">
            {block.frames.map((frame, idx) => (
              <div key={frame.id} className="flex w-[240px] shrink-0 flex-col rounded-lg border border-[color:rgba(255,255,255,0.07)] bg-[#1c1c1a]">
                {/* Frame header */}
                <div className="flex items-center justify-between border-b border-[color:rgba(255,255,255,0.07)] px-3 py-2">
                  <span className="font-mono text-[15px] font-medium text-[color:var(--cs-accent)]">
                    FRAME {String(idx + 1).padStart(2, '0')}
                  </span>
                  <button
                    onClick={(e) => { e.stopPropagation(); setOpenFrameId(frame.id) }}
                    className="flex items-center gap-1 rounded bg-[color:var(--cs-accent)]/20 px-2 py-1 text-[15px] text-[color:var(--cs-accent2)] hover:bg-[color:var(--cs-accent)]/40"
                ...
                <div className="flex flex-1 gap-2 p-2" style={{ minHeight: 120 }}>
                  {/* Image area */}
                  <label
                    className="relative flex w-1/2 cursor-pointer items-center justify-center overflow-hidden rounded bg-black/40 text-[14px] text-[#888780]"
                    onDrop={(e) => {
                      e.preventDefault()
                  ...
                  <textarea
                    ...
                    className="w-1/2 resize-none rounded bg-black/40 p-2 text-[15px] leading-[1.5] text-white outline-none placeholder:text-[#555450]"
          ...
          {openFrame && (
            ...
            >
              <motion.div
                ...
                className="flex h-[80vh] w-[min(800px,92vw)] flex-col overflow-hidden rounded-lg border border-[color:var(--cs-accent)]/50 bg-[#111110]"
                style={{ boxShadow: '0 0 80px color-mix(in srgb, var(--cs-accent) 20%, transparent)' }}
                onClick={(e) => e.stopPropagation()}
              >
                <div className="flex items-center justify-between border-b border-[color:rgba(255,255,255,0.07)] px-6 py-4">
                  <div>
                    <div className="font-mono text-[13px] font-medium uppercase tracking-[0.06em] text-[color:var(--cs-accent)]">Storyboard Frame</div>
                    <input
                  ...
                  <button onClick={() => setOpenFrameId(null)} className="text-[#888780] hover:text-white">
                    <X size={18} />
                  </button>
                </div>
                ...
                {/* Image preview */}
                {openFrame.image && (
                  <div className="border-b border-[color:rgba(255,255,255,0.07)] bg-black/40 p-4">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                ...
                <div className="border-b border-[color:rgba(255,255,255,0.07)] px-6 py-3">
                  <div className="mb-1 font-mono text-[13px] font-medium uppercase tracking-[0.06em] text-[#888780]">Quick Notes</div>
                  <textarea
                    defaultValue={openFrame.notes}
                    placeholder="- Bullet notes…"
                    className="w-full resize-none bg-transparent text-[15px] leading-[1.5] text-white outline-none placeholder:text-[#555450]"
                    rows={3}
                    onBlur={(e) =>
                ...
                <div className="flex-1 overflow-auto px-6 py-4">
                  <div className="mb-2 font-mono text-[13px] font-medium uppercase tracking-[0.06em] text-[#888780]">Detailed Notes</div>
                  <textarea
                    autoFocus
                    key={openFrame.id}
                    defaultValue={openFrame.detailedNotes ?? ''}
                    placeholder="Write in-depth scene breakdown, direction notes, dialogue, action beats…"
                    className="h-full min-h-[200px] w-full resize-none bg-transparent text-[15px] leading-[1.5] text-white outline-none placeholder:text-[#555450]"
                    onBlur={(e) =>
                      setFrames((f) =>
