      ...
      <BlockWrapper block={block} kind="storyboard" onContextMenu={onContextMenu}>
        <div className="flex h-full w-full flex-col overflow-hidden rounded-md border border-[#2a2a2a] bg-[#141412] shadow-lg">
          ...
          <div data-no-drag className="flex items-center justify-between border-b border-[#2a2a2a] bg-[#1a1a18] px-4 py-2">
            <span className="font-mono text-[13px] font-medium uppercase tracking-[0.06em] text-amber-500">Storyboard</span>
            <div className="flex gap-1">
              <button
                className="flex items-center gap-1 rounded bg-[#2a2a2a] px-2 py-1 text-[15px] text-white hover:bg-[#3a3a3a]"
                onClick={(e) => { e.stopPropagation(); sequence() }}
              >
                <Film size={11} /> Sequence
              </button>
              <button
                className="flex items-center gap-1 rounded bg-amber-600 px-2 py-1 text-[15px] text-black hover:bg-amber-500"
            ...
            {block.frames.map((frame, idx) => (
              <div key={frame.id} className="flex w-[240px] shrink-0 flex-col rounded-lg border border-[#2a2a2a] bg-[#0e0e0d]">
                ...
                <div className="flex items-center justify-between border-b border-[#2a2a2a] px-3 py-2">
                  <span className="font-mono text-[15px] font-medium text-amber-500">
                    FRAME {String(idx + 1).padStart(2, '0')}
                  </span>
                  <button
                    onClick={(e) => { e.stopPropagation(); setOpenFrameId(frame.id) }}
                    className="flex items-center gap-1 rounded bg-amber-600/20 px-2 py-1 text-[15px] text-amber-400 hover:bg-amber-600/40"
                ...
                <div className="flex flex-1 gap-2 p-2" style={{ minHeight: 120 }}>
                  {/* Image area */}
                  <label
                    className="relative flex w-1/2 cursor-pointer items-center justify-center overflow-hidden rounded bg-black/40 text-[14px] text-neutral-500"
                    onDrop={(e) => {
                      e.preventDefault()
                  ...
                  <textarea
                    ...
                    className="w-1/2 resize-none rounded bg-black/40 p-2 text-[15px] leading-[1.5] text-white outline-none placeholder:text-neutral-600"
            ...
            >
              ...
              >
                <div className="flex items-center justify-between border-b border-[#2a2a2a] px-6 py-4">
                  <div>
                    <div className="font-mono text-[13px] font-medium uppercase tracking-[0.06em] text-amber-500">Storyboard Frame</div>
                    <input
                ...
                <div className="border-b border-[#2a2a2a] px-6 py-3">
                  <div className="mb-1 font-mono text-[13px] font-medium uppercase tracking-[0.06em] text-neutral-500">Quick Notes</div>
                  <textarea
                    defaultValue={openFrame.notes}
                    placeholder="- Bullet notes…"
                    className="w-full resize-none bg-transparent text-[15px] leading-[1.5] text-white outline-none placeholder:text-neutral-600"
                    rows={3}
                    onBlur={(e) =>
                ...
                <div className="flex-1 overflow-auto px-6 py-4">
                  <div className="mb-2 font-mono text-[13px] font-medium uppercase tracking-[0.06em] text-neutral-500">Detailed Notes</div>
                  <textarea
                    autoFocus
                    key={openFrame.id}
                    defaultValue={openFrame.detailedNotes ?? ''}
                    placeholder="Write in-depth scene breakdown, direction notes, dialogue, action beats…"
                    className="h-full min-h-[200px] w-full resize-none bg-transparent text-[15px] leading-[1.5] text-white outline-none placeholder:text-neutral-600"
                    onBlur={(e) =>
                      setFrames((f) =>
