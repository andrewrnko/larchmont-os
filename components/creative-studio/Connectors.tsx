          ...
          <g key={c.id} data-connector={c.id} className="pointer-events-auto">
            ...
            {isSelected && (
              <foreignObject x={mid.x - 12} y={mid.y - 12} width={24} height={24}>
                <button
                  className="flex h-6 w-6 items-center justify-center rounded-full border border-red-600 bg-red-900 text-[14px] text-red-300 shadow-lg hover:bg-red-800"
                  onClick={(e) => {
                    e.stopPropagation()
            ...
            {editing === c.id && (
              <foreignObject x={mid.x - 60} y={mid.y - 10} width={120} height={22}>
                <input
                  ...
                  className="w-full rounded bg-black/90 px-1 text-[14px] text-white outline-none"
                  onBlur={(e) => {
